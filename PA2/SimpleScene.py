...
# Dragging state
isDrag = 0
control_points = []  # List to store control points
animStartTime = None
animating = False
rotateCow = False
rotateStartTime = None
...
def drawCow(_cow2wld, drawBB):
    glPushMatrix()  # Push the current matrix of GL into stack. This is because the matrix of GL will change while drawing cow.
...
def display():
    global cameraIndex, cow2wld, animStartTime, control_points, animating, rotateCow, rotateStartTime
    ...
    if animating and len(control_points) == 6:
        animTime = glfw.get_time() - animStartTime
        cycle_duration = 6.0  # Total time to complete one cycle (1 second per segment)
        num_cows = 6
        t = (animTime % cycle_duration) / cycle_duration

        # Determine the current segment and local t in that segment
        segment_duration = cycle_duration / num_cows
        segment_index = int(t * num_cows)
        local_t = (t * num_cows) % 1.0

        p0 = control_points[segment_index]
        p1 = control_points[(segment_index + 1) % num_cows]

        # Linear interpolation between p0 and p1
        position = (1 - local_t) * p0 + local_t * p1

        # Create transformation matrix for the cow
        T = np.eye(4)
        setTranslation(T, position)
        cow2wld = T

        if animTime > cycle_duration * 3:
            animating = False
            rotateCow = True
            rotateStartTime = glfw.get_time()
            control_points.clear()  # Clear control points after animation

    if rotateCow:
        rotateTime = glfw.get_time() - rotateStartTime
        if rotateTime < 2.0:
            angle = (rotateTime / 2.0) * 360  # Rotate 360 degrees in 2 seconds
            T = np.eye(4)
            setTranslation(T, getTranslation(cow2wld))
            glPushMatrix()
            glLoadMatrixd(T.T)
...
def onMouseButton(window, button, state, mods):
    global isDrag, V_DRAG, H_DRAG, control_points, animStartTime, animating
    GLFW_DOWN = 1
    GLFW_UP = 0
    ...
    if button == glfw.MOUSE_BUTTON_LEFT:
        if state == GLFW_DOWN:
            ...
            control_points.append(cowPickPosition)  # Add control point
            if len(control_points) == 6:
                animStartTime = glfw.get_time()  # Start animation if six control points are specified
                animating = True
...
def onMouseDrag(window, x, y):
    ...
    if isDrag:
        ...
        if cursorOnCowBoundingBox:
            ray = screenCoordToRay(window, x, y)
            pp = pickInfo
            # Create a plane perpendicular to the ray direction, passing through the cow pick position
            dragging_plane_normal = np.array((1, 0, 0))  # Plane normal for vertical drag (x-axis)
            p = Plane(dragging_plane_normal, pp.cowPickPosition)
            intersection = ray.intersectsPlane(p)

            if intersection[0]:
                currentPos = ray.getPoint(intersection[1])
                ...
                # pp.cowPickPosition = currentPos  # Update the pick position for continuous dragging
